...
Мы уже познакомились с вектором и его устройством в памяти в главе [vector.md](../rust-basics/vector.md "mention").
...
fn main() {
    ...
    // Удаляем рядом стоящие дубликаты
    v.dedup();
...
В отличии от вектора, двусвязанный список позволяет вставлять/удалять элементы в середину и начало списка.
...
Двусвязанный список позволяет вставлять и удалять элементы в середине и в начале списка, но его недостатком является то, что операция доступа к элементу по индексу имеет сложность O(n/2) в отличии от сложность O(1) у вектора.
...
Операции вставки и поиска для `BTreeMap` работают немного медленнее, чем для `HashMap`, особенно при большом размере словаря (больше тысячи элементов). Однако `BTreeMap` хранит элементы в порядке отсортированном по ключу, что может быть удобно в некоторых сценариях использования.
