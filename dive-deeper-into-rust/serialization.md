...
serde не содержит функциональности для сериализации в какие-либо форматы, например JSON или XML, но предоставляет трэйты и процедурные макросы, которые позволяют атвтоматически сгененирировать для пользовательских типов набор методов, выдающих метаинформацию о типе, а так же позволяющих читать поля и записывать в них значения. Уже на основе этих методов, другие библиотеки, работающие с конкретными форматами (JSON, XML, YAML), производят сериализацию и десериализацию.
...
Чтобы увидеть код реализаций `Deserialize` и `Serialize` для нашей структуры `Employee`, создадим такую простую программку, состоящую по сути из одного лишь объявления структуры.
...
Как видите, реализация `Serialize` очень простая. Она состояит из метода, который выдаёт метаинформацию о структуре: имя труктуры и перечень полей.

Реализация `Deserialize` куда сложнее. В рамках её генерируется визитор (шаблон Visitor), который используется для обхода полей структуры.

</details>

## Сериализация в JSON

Давайте разберёмся как нам сериализовать нашу структуру `Employee` в JSON.

Для начала нам понадобится добавить зависимость на [serde\_json](https://crates.io/crates/serde_json) в `Cargo.toml`:
...
* [serde\_json::to\_string](https://docs.rs/serde_json/latest/serde_json/fn.to_string.html) — сериализует объект в JSON строку
* [serder\_json::from\_str](https://docs.rs/serde_json/latest/serde_json/fn.from_str.html) — десериализует объект из JSON строки
...
Так же в крэйте serde\_json определёно перечисление [Value](https://docs.rs/serde_json/latest/serde_json/enum.Value.html), которое по сути является отображением на JSON типы.
...
Как видите, благодаря serde, API работы с XML очень похож на API для работы с JSON.
...
```
"вариант_перечисления": { поля объекта }
```
...
Альтернативно, вместо оборачивания в поле, можно непосредственно в объект добавить тег, который будет хранить вариант парачисления. Какое имя будет носить этот тег задаётся при помощи аннотации `#[serde(tag="имя")]`.
...
Если имя хотим чтобы имя поле в сериализованном виде отличалось от имени поля в структуре, то нам поможет аннотация `#[serde(rename = "сериализованное_имя")]`.
...
Пример: указание, что при сериализации и десериализации должна использоваться верблюжья нотация (camelCase).
