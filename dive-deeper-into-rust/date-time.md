...
Однако, у системного таймера есть проблема: если вызвать `SystemTime::now()`, и сразу после этого вызвать `SystemTime::now()` еще раз, то есть вероятность, что время полученное в результате второго вызова, будет раньше чем время полученное в результате первого вызова. Почему так происходит — отдельная история, но главное что нам нужно знать: не следует использовать системный таймер для того, чтобы замерять время выполнения участков кода.
...
Важно отметить, что типы данных из библиотеки chrono поддерживают сериализацию по средствам [serde](https://crates.io/crates/serde), однако эту функциональность надо отдельно включить при помощи фичи "serde".
...
Соответственно `NaiveDateTime` — это комбинация `NaiveDate` и `NaiveTime`.
...
* [FixedOffset](https://docs.rs/chrono/latest/chrono/struct.FixedOffset.html) — хранит фиксированный сдвиг в секундах относительно UTC
...
Кроме получения текущего времени, объект `DateTime` можно создать:
