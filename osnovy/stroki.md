...
Функция конструктор `String::new` просто создаёт новый объект `String` с пустым строковым буфером. Это может нужно, например, для того чтобы передать эту строку в функцию, которая наполнит её.
