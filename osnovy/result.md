...
pub trait Error: Debug + Display {
    ...
    // Устаревший, использовать fmt::Debug вместо него
    fn description(&self) -> &str { ... }

    // Устаревший, использовать source
...
Если мы вызываем функцию, которая возвращает `Result`, и при этом не присваиваем её результат какой-то переменной, то компилятор выдаст предупреждение, поскольку будет считать, что мы просто забыли проверить результат на возможную ошибку.
