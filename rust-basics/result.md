...
В примере выше, в качестве ошибки, мы использовали просто строку с описание проблемы. Разумеется, такой подход крайне неудобен для программной обработки ошибки. Гораздо удобнее хранить информацию об ошибке в перечеслении, в котором каждый элемент представляет отдельную проблему.
