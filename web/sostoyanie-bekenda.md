...
Для работы с таким состоянием приложения в Axum существует обёртка [State](https://docs.rs/axum/latest/axum/extract/struct.State.html). Принцип работы следующий:

1\) Мы создаём объект произвольной структуры, и добавляем его в конфигурацию роутера при помщи метода [with\_state](https://docs.rs/axum/latest/axum/struct.Router.html#method.with_state).
...
#[tokio::main]
async fn main() {
    let my_app_data = Arc::new(MyAppData { ... });
...
// Эта структура будет определять состояние нашего приложения
struct AppState {
    counter: AtomicU64,
}
...
* Axum приложение может иметь несколько роутеров, и для каждого из них можно указать свой объект состояния, что мы увидим в [следующей главе](routing.md#sliyanie-routerov)
