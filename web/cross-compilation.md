...
Так же, если вы работает под Linux, но версия GLIBC (стандартная библиотека C используемая по умолчанию в большинстве дистрибутивов Linux) на вашей системе старше, чем версия GLIBC на целевом Linux сервере, то скорее всего, исполняемый бинарный файл собранный под вашу систему, так же не запуститься на целевом Linux сервере.

Собрать программу на Rust под другую платформу можно двумя путями:

* Произвести сборку на таком же окружении как и целевой Linux сервер. Это может быть как отдельная физическая машина, так и Docker контейнер или виртуальная машина, где запущена такая же окружение, как и у вашей целевой платформы.
* Произвести кросс-компиляцию под другую целевую платформу.

Со сборкой на другой машине, или в Docker контейнере всё более-менее и так понятно. А с кросс-компиляцией мы будем разбираться дальше в этой главе.
...
Например, собрать программу для Windows x86\_64 из под Linux Ubunu относительно не трудно, но только потому что в репозитории пакетов Ubuntu имеется нужный пакет с библиотекой C и линкером, который умеет собирать исполняемый файлы для Windows:

```shellscript
# Установить MinGW C++ тулчейн  для Windows, который содержит линкер бибилотеку C
...
3. Добавите путь к  корневой папке с Zig SDK (папка в которой находится исполняемый файл `zig`) в системный путь: переменная `PATH` на Unix-подобных ситемах, и `Path` — на Windows.
...
Предположим, что вы работаете на Linux или MacOS, и хотите собрать программу для Windows при помощи zigbuild. Всё нужно:
...
Для того при сборке чтобы задать конкретную версию GLIBC, укажите её после точки в имени таргета. Например, чтобы слинковать с GLIBC 3.34, используйте команду сборки:
...
Например, если вы используете Ubuntu Linux, то вам будет необходимо устновить пакеты `musl-tools` и `musl-dev`:

```
sudo apt install musl-tools musl-dev
```

после чего вы сможете собрать свою программу под Linux, но слинковав её с MUSL:
...
Например, на момент написания этого текста (Rust 1.92) в экосистеме Rust отсутствовал драйвер для СУБД Oracle, который бы был полностью написан на Rust. Имеющийся крэйт [oracle](https://crates.io/crates/oracle), является просто обёрткой над библиотекой написанной на C, и слинкованной с GLIBC. Причём код бибилотеки закрыт, что исключает возможность самостоятельно скомпилировать её, и статически слинковать с программой на Rust. Поэтому, если ваше приложение должно работать с СУБД Oracle, вам придётся использовать GLIBC.
