...
Экосистема Axum предлагает предлагает крэйт [axum-test](https://crates.io/crates/axum-test), который позволяет проводить полноценное тестирование эндпоинтов минуя сетевую часть. Это очень удобно так как позволяет запускать тесты быстрее, и не переживать о коллизии портов.
...
Давайте немного модифицируем наш пример Hello-сервера так, чтобы его было удобно тестировать, а так же добавим тест для эндпоинта.
...
#[cfg(test)] // Чтобы тесты компилировались только для тестового запуска
mod test {
