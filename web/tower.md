...
Первое, что нам необходимо сделать: добавить в `Cargo.toml` зависимости [tower](https://crates.io/crates/tower) и [tower\_http](https://crates.io/crates/tower-http).

<pre class="language-toml"><code class="lang-toml">[package]
name = "test_axum"
version = "0.1.0"
edition = "2024"
...
axum = "0.8"
<strong>tower = { version = "0.5", features = ["full"] }
</strong><strong>tower-http = { version = "0.6", features = ["cors", "fs"] }
</strong>
tracing = "0.1"
...
`Service` астрагируется над некой асинхронной функцией, которая принимает, в качестве аргумента, объект некоторого генерик типа `Request`, и возвращает либо некий генерик `Response`, либо ошибку.
...
Хорошо, мы убедились, что такие сущности как обработчики запросов и мидлваре можно абстрагирують через трэйт `Service`, но что это нам даёт?

Дело в том, что если различные обработчики абстрагированы до единого интерфейса, то они превращаются в универсальные строительные блоки, из которых можно строить цепочки для обработки запросов. То есть можно создать универсальные реализации для таких вещей как retry, back-pressure, фильтрация запросов, CORS, компрессия, таймауты и т.д. И именно это и преставляет из себя библиотека tower: трэйт `Service` и множество готовых универсальных строительных блоков, построенных на его основе.

И вот теперь пришло время сказать, что в Axum позволяет использовать реализации `Service` и в качестве обработчиков запросов, и в качестве мидлваре.

## Обработчик запроса как Service

До этого момент, мы создавали эндпоинты путём регистрации функций-обработчиков в роутере при помощи функции [route](https://docs.rs/axum/latest/axum/struct.Router.html#method.route). При этом роутер предоставляет еще один метод — [route\_service](https://docs.rs/axum/latest/axum/struct.Router.html#method.route_service), которые регистрирует `Service` в качестве обработчика:
...
{% hint style="info" %}
[std::convert::Infallible](https://doc.rust-lang.org/std/convert/enum.Infallible.html) — тип ошибки, объявленный как пустое перечисление, что делает невозможным создание объекта этого типа. Он используется в ситуациях, когда функция объявленная в трэйте, возвращает результат завёрнутый в `Result`, однако конкретная реализация трэйта  подразумевает, что функция никогда не вернёт ошибку.&#x20;
{% endhint %}

Давайте, напишем наш традиционный hello сервер, но на этот раз в качестве обработчика запроса будет выступать не функция, а объект типа реализующего трэйт `Service`.
...
Не трудно заметить, что создание обработчика запроса в виде отдельного типа, реализующего `Service`, является более трудоёмким, по сравнению с созданием обработчика из функции. Однако, такой подход предоставляет куда больше гибкости, так как позволяет нашему объект-обрабочику иметь внутренее состояние.

## Мидлваре как Service

Как мы знаем из прошлой главы, мидлваре регистрируется в роутере при помощи метода [layer](https://docs.rs/axum/latest/axum/struct.Router.html#method.layer), который мы наконец-то готовы разобрать подробнее. Он имеет вид:
...
Здесь [Layer](https://docs.rs/tower-layer/latest/tower_layer/trait.Layer.html) — трэйт для типов, которые возвращают объект реализующий трэйт Service. Можно сказать, что `Layer` — это фабрика для `Service`.

```rust
pub trait Layer<S> {
    type Service;

    // Принимает в качестве аргумента (inner) объект следующего в цепочке мидлваре
    // или обработчика запроса и возвращает свой объект мидлваре.
    // Если для сервиса нужен объект состояния бекенда, то он содержится в self.state
    fn layer(&self, inner: S) -> Self::Service;
}
```

Давайте, перепишем в виде сервиса наш мидлваре из прошлой главый, который логирует время выполнения обработчика запроса.
...
struct ExecTimeLogService<S> {
    // Следующий в цепочке мидлваре, или обработчик запроса
    next_handler: S,
}
...
Как и в ситуации с обработчиками запросов, создание мидлваре в виде типа, реализующего `Service`, даёт больше гибкости, так как позвояляет объекту мидлваре иметь состояние.
...
<pre class="language-rust"><code class="lang-rust">use axum::Router;
use tower_http::services::ServeFile;

#[tokio::main]
async fn main() {
    let app = Router::new()
<strong>        .route_service("/index", ServeFile::new("index.html"));
</strong>    let listener = tokio::net::TcpListener::bind("0.0.0.0:8080").await.unwrap();
    axum::serve(listener, app).await.unwrap();
}
</code></pre>
...
<pre class="language-rust"><code class="lang-rust">use axum::{Router, routing::get};
...
<strong>        .layer(CorsLayer::new().allow_origin(allowed_origins));
</strong><strong>
</strong>    let listener = tokio::net::TcpListener::bind("0.0.0.0:8080").await.unwrap();
    axum::serve(listener, app).await.unwrap();
}
...
К этому моменту вам должно быть понятно как использвать сервисы и мидлваре основанные на tower. Напоследок, перечислям некоторые наиболее частоиспользуемые из них.
...
* мидлваре [TimeoutLayer](https://docs.rs/tower-http/latest/tower_http/timeout/struct.TimeoutLayer.html) — позволяет установить максимальное время выполнения эндпоинта, после истечения которого будет возвращён заданный HTTP код
