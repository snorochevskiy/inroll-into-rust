...
Для бекенд приложений с REST API, хорошие практикой считается предоставлять эндпоинт, который возвращает описание доступных эндпоинтовы в формате [OpenAPI](https://swagger.io/specification/).

Open API описание позволяет не только понять какие эндпоинты предоставляет бекенд приложение, но и автоматически сгенерирововать клиент для работы с этими эндпоинтами при помощи таких инструментов как [Insomnia](https://insomnia.rest/) и [Postman](https://www.postman.com/).
...
Для того, чтобы Utoipa мог сгенерировать Open API описание из функции обработчика запросов, эту функцию надо пометить аннотацие [path](https://docs.rs/utoipa/latest/utoipa/attr.path.html), указав HTTP метод, URL путь, варианты ответа и т.д.
...
async fn main() {
    ...
    // Добавляем к роутеру, сгенерированного из OpenAPI роутера,
...
async fn main() {
    ...
    let (router, api) = open_api.split_for_parts();
    let app = router.route("/api-doc", get(async move || { api.to_pretty_json().unwrap() }));
    let listener = tokio::net::TcpListener::bind("0.0.0.0:8080").await.unwrap();
    axum::serve(listener, app).await.unwrap();
}
```
...
3. Соответствующий крэйты для SwaggerUI / Redoc / RapiDoc / Scalar предоставляют специальный Axum сервис, который мы просто подключаем в роутер.
4. При переходе на URL, по которому web-инструмент зарегистрирован в роутере, подгружается веб страница со SwaggerUI (или другим приложением), которая по Open API описанию создаёт клиент позволяющий вызов API прямо с веб страницы.
...
async fn main() {
    ...
    let app = router
<strong>        .merge(SwaggerUi::new("/swagger-ui").url("/api-docs/openapi.json", api.clone()))
</strong><strong>        .merge(Redoc::with_url("/redoc", api.clone()))
</strong><strong>        .merge(RapiDoc::new("/api-docs/openapi.json").path("/rapidoc"))
</strong><strong>        .merge(Scalar::with_url("/scalar", api));
</strong>
    let listener = tokio::net::TcpListener::bind("0.0.0.0:8080").await.unwrap();
    axum::serve(listener, app).await.unwrap();
}
...
<figure><img src="../.gitbook/assets/swagger.png" alt="" width="563"><figcaption></figcaption></figure>

* Redoc: [http://localhost:8080/redoc](http://localhost:8080/redoc)

<figure><img src="../.gitbook/assets/redoc.png" alt="" width="563"><figcaption></figcaption></figure>

* RapiDoc: [http://localhost:8080/rapidoc](http://localhost:8080/rapidoc)

<figure><img src="../.gitbook/assets/rapidoc.png" alt="" width="563"><figcaption></figcaption></figure>

* Scalar: [http://localhost:8080/scalar](http://localhost:8080/scalar)

<figure><img src="../.gitbook/assets/scalar.png" alt="" width="563"><figcaption></figcaption></figure>
...
// Тип, который мы будем использовать, что добавленимя в OpenAPI документ
// информации об аутентификаии
...
<figure><img src="../.gitbook/assets/swagger_auth.png" alt="" width="563"><figcaption></figcaption></figure>
