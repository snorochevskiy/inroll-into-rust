...
* Экстрактор для инжекции тела запроса, переданного как JSON документ — [Json](axum-basics.md#chtenie-tela-zaprosa)
...
Теперь давайте рассмотрим довольно распостранённый пример: экстрактор, который извлекает ID сессии из HTTP заголовка.
...
// Стояние приложения, с хранилищем сессий в нём
struct AppState {
...
#[tokio::main]
async fn main() {
    // Предпоготовленное хранилище сессий для тестирования нашего экстрактора
...
Как вы уже могли догадаться, типы `Json` и `Form<FormData>`, которые мы рассматривали в [разделе про чтение данных запроса](axum-basics.md#chtenie-tela-zaprosa),  реализуют `FromRequest`, что позволяет им инжектиться в функцию обрабочтик.
...
impl<S: Send + Sync, D: DeserializeOwned> FromRequest<S> for AnyFormat<D> {
    ...
    async fn from_request(
        ...
        // Извлекаем из запроса заголовок и тело
