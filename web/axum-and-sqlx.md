...
Теперь когда мы разобрались как работать с SQLx, давайте посмотрим как использовать его cовместно с веб сервером, основанным на Axum.

Расширим наш test\_sqlx проект (который мы создали в главе [sqlx.md](sqlx.md "mention")), добавив в него Axum сервер. Для простоты примера, пускай наш сервер будет предоставлять только два эндпоинта:
...
<strong>axum = "0.8"
</strong><strong>serde = { version = "1", features = ["derive"] }
</strong><strong>serde_json = "1"
</strong></code></pre>
...
Объект пула соединений, необходимый для выполнения запроса в БД, мы будем передавать как аргумент фукции. Итак, наш файл `src/persist.rs`:
...
pub async fn create_accounts(
    ...
    // Функция currval(сиквенс), которая возвращает последнее полученное значение сиквенса,
...
Таким образом файл `server.rs` будет иметь вид:
...
async fn list_accounts(
    ...
    // вызова функции, вычитывающей все аккаунты из БД.
...
В главном файле `main.rs` мы просто вызовем функцию запуска сервера:
...
<pre><code><strong>$ curl -i http://localhost:8080/accounts
...
<pre><code><strong>$ curl -i -X POST \
</strong><strong>    -H "Content-Type: application/json" \
</strong><strong>    -d '{"owner_name": "Acc-1", "init_balance": 1000.0}' \
</strong><strong>    http://localhost:8080/accounts
</strong>HTTP/1.1 200 OK
content-type: application/json
content-length: 49
...
Как видите, объект состояния — именно то место, где следует хранить долгоживущие сущности, которые могут использоваться разными эндпоинтами.
