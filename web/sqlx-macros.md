...
1. Вместо функций `query_as`, `query_scalar` и `query` мы должны использовать соответствующие макросы: [query\_as!](https://docs.rs/sqlx/latest/sqlx/macro.query_as.html), [query\_scalar!](https://docs.rs/sqlx/latest/sqlx/macro.query_scalar.html) и [query!](https://docs.rs/sqlx/latest/sqlx/macro.query.html). Принцип их использования такой же, как и у их собратьев — функций.\
...
2. Поэтому запускаем эту команду:\
   ...
   Утилита подключится к базе данных, и с её помощью провалидирует все SQL запросы испозуемые в макросах `query_as!`, `query_scalar!` и `query!`.\
...
## query\_as!, query\_scalar! и query!
...
Так же макрос `sqlx::query_as!` принимает минимум два аргумента: структура, в виде ообъектов которой будет представлен результат запроса, и сам SQL запрос. Напомним, функция `query_as` имеет только один аргумент — SQL запрос. Если код запроса содержит аргументы, то значения для них передаются сразу после самого запроса.
...
Если команда `cargo sqlx prepare` завершилась успешно, томожно запустить программу:
...
## query\* функциц vs. query\*! макросы
